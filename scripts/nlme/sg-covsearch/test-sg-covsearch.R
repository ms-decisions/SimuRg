...
.stage2_mock_fit_ll <- function(ll_map) {
  force(ll_map)
  function(model, data, headers, theta, ruv, re, occ, covs, project_name,
    ...
    ll <- ll_map[[project_name]]
    if (is.null(ll)) {
      stop(sprintf("Missing mock LL for project %s", project_name))
    }
    list(
      GFO = list(
        OFV = data.frame(LL = as.numeric(ll)),
        SUMTAB = data.frame(PAR = character(0), VALUE = numeric(0), stringsAsFactors = FALSE),
...
test_that("Stage 2 generates full candidate cross-product when test_pairs is NULL", {
  ...
  res <- stepwise_covariate_selection(
    ...
    test_pairs = NULL,
    run_backward = FALSE,
    fit_function = .stage2_mock_fit_ll(ll_map)
  )
...
test_that("Stage 2 derives NA-safe medians for continuous covariates", {
  expect_equal(.na_safe_median(.covsearch_cotab_fixture$WT), .covsearch_expected_refs_fixture$WT)
  expect_equal(.na_safe_median(.covsearch_cotab_fixture$AGE), .covsearch_expected_refs_fixture$AGE)
...
.stage3_mock_fit <- function(ofv_map, sumtab_map = list(), fail_projects = character(0)) {
  force(ofv_map)
  force(sumtab_map)
  force(fail_projects)
  function(model, data, headers, theta, ruv, re, occ, covs, project_name,
...
test_that("Stage 3 removes accepted candidate from remaining set", {
  ...
  tested_cl_wt <- res$forward$history[
    res$forward$history$parameter == "CL" & res$forward$history$covariate == "WT",
    ,
    drop = FALSE
  ]
...
test_that("Stage 3 updates theta INIT only from accepted step fits", {
  ...
  ka_row <- res$final_gco$theta[res$final_gco$theta$NAME == "ka", , drop = FALSE]
...
test_that("Stage 4 removes least important removable term first", {
  ...
  res <- stepwise_covariate_selection(
    ...
    p_backward = 0.01,
    run_backward = TRUE,
    fit_function = .stage3_mock_fit(ll_map)
  )
