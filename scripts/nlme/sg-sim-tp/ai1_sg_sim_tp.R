...
sg_sim_tp <- function(
  ...
  if (!is.null(bands_i) && length(bands_i) == 2) {
    p <- p +
      geom_ribbon(aes(ymin = !!sym(bands_i[1]), ymax = !!sym(bands_i[2]),
                      fill = if (!is.null(fill_i)) !!sym(fill_i) else NULL),
                  alpha = 0.3)
  }
  ...
  if (!is.null(lperc_i) && !is.null(uperc_i) && needs_summary) {
    p <- p +
      geom_ribbon(aes(ymin = y_lower_perc, ymax = y_upper_perc,
                      fill = if (!is.null(fill_i)) !!sym(fill_i) else NULL),
                  alpha = 0.3)
  }
  ...
  line_aes <- aes(
    color = if (!is.null(col_i)) !!sym(col_i) else NULL,
    linetype = if (!is.null(lty_i)) !!sym(lty_i) else NULL,
    group = if (length(group_cols) > 0) interaction(!!!syms(group_cols)) else NULL
  )
  
  # Add line layer
  p <- p + geom_line(line_aes, linewidth = 0.8)
  
  # Add points if requested
  if (add_points > 0) {
    point_aes <- aes(
      color = if (!is.null(col_i)) !!sym(col_i) else NULL,
      shape = if (!is.null(shp_i)) !!sym(shp_i) else NULL
    )
    p <- p + geom_point(point_aes, size = add_points)
  }
