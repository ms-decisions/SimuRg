...
#####--------------- Simulations ---------------#####
### Basic
sim1 <- fun_ForSim(mod_ex, et_base, seq(0, 168, 0.1), vars_i = "Cc", cov_i = c("IGFR", "POPN"))
sim2 <- fun_ForSim(mod_ex, et_base, seq(0, 168, 0.1), cov_i = c("IGFR", "POPN"))

### BSV and RUV
sim3 <- fun_ForSim(mod_ex, et_base, seq(0, 168, 0.1), vars_i = c("Cc", "Cc_ResErr"), cov_i = c("IGFR", "POPN"), omega_i = omega, sigma_i = sigma)
sim4 <- fun_ForSim(mod_ex, et_base, seq(0, 168, 0.1), vars_i = c("Cc", "Cc_ResErr"), cov_i = c("IGFR", "POPN"), omega_i = omega, sigma_i = sigma, nsub = 10, aggr_id = T)

### Uncertainty
sim5 <- fun_ForSim(mod_ex, et_base, seq(0, 168, 0.1), vars_i = c("Cc", "Cc_ResErr"), cov_i = c("IGFR", "POPN"), thetamat_i = thetamat, npop = 10, aggr_id = T) # 
sim6 <- fun_ForSim(mod_ex, et_base, seq(0, 168, 0.1), vars_i = c("Cc", "Cc_ResErr"), cov_i = c("IGFR", "POPN"), thetamat_i = thetamat, nsub = 10, npop = 10, aggr_id = T) # 

### BSV, RUV, Uncertainty
sim7 <- fun_ForSim(mod_ex, et_base, seq(0, 168, 0.1), vars_i = c("Cc", "Cc_ResErr"), cov_i = c("IGFR", "POPN"), 
                   omega_i = omega, sigma_i = sigma, thetamat_i = thetamat, nsub = 10, npop = 10, aggr_id = T)

### Time-varying covariates
sim8 <- fun_ForSim(mod_ex, et_tvar_cov, vars_i = c("Cc", "IGFRCOV", "CL"), cov_i = c("IGFR", "POPN"))


#####--------------- Visualization ---------------#####
fun_Plot <- function(sim_i, aggr = F){
  if(!aggr){
    p_i <- ggplot(data = sim_i, aes(x = time, y = VALUE, group = interaction(id, sim.id))) +
