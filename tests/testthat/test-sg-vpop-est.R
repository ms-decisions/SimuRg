...
test_that("sg_vpop_est returns correct structure with simple continuous data", {
  ...
  expect_equal(colnames(result$datagen), colnames(test_data))
...
test_that("sg_vpop_est works with mixed continuous and categorical data", {
  ...
  expect_true(is.data.frame(result$datagen))
  expect_equal(ncol(result$datagen), ncol(test_data))
  expect_equal(nrow(result$datagen), nrow(test_data))
...
test_that("sg_vpop_est respects nobj parameter", {
  ...
  output <- sg_vpop_est(data_i = test_data, nobj = 50, diag_plots = FALSE, seed = 123)

  result <- output[[1]]
  expect_equal(nrow(result$datagen), 50)
})
...
test_that("sg_vpop_est respects npop parameter", {
  ...
  output <- sg_vpop_est(data_i = test_data, npop = 2, diag_plots = FALSE, seed = 123)

  result <- output[[1]]
  expect_equal(nrow(result$datagen), 200)
})
...
test_that("sg_vpop_est handles minnumlev parameter", {
  ...
  # With minnumlev = 3, low_level should be treated as categorical
...
test_that("sg_vpop_est handles NA values by removing rows", {
  ...
  result <- output[[1]]
  expect_true(is.data.frame(result$datagen))
  expect_equal(nrow(result$datagen), 45)  # Should match number of non-NA rows
})
...
test_that("sg_vpop_est fixed seed mode generates single dataset", {
  ...
  output <- sg_vpop_est(data_i = test_data, seed = 456, diag_plots = FALSE)
...
test_that("sg_vpop_est search mode generates multiple datasets", {
  ...
  test_data <- data.frame(
    x1 = rnorm(100, mean = 10, sd = 2),
    x2 = rnorm(100, mean = 20, sd = 3),
    x3 = rnorm(100, mean = 5, sd = 1)
  )

  # Search mode with nds = 3
  output <- sg_vpop_est(data_i = test_data, seed = NA, nds = 3,
                        tg_corrdif = 0.15, diag_plots = FALSE)
...
test_that("sg_vpop_est computes correlation difference metrics", {
  ...
  output <- sg_vpop_est(data_i = test_data, seed = 789, diag_plots = FALSE)
...
test_that("sg_vpop_est computes JSD for categorical variables", {
  ...
  output <- sg_vpop_est(data_i = test_data, seed = 321, diag_plots = FALSE)
...
test_that("sg_vpop_est checks for exact duplicates", {
  ...
  output <- sg_vpop_est(data_i = test_data, seed = 111,
...
test_that("sg_vpop_est generates correct plots with diag_plots = TRUE", {
  ...
  test_data <- data.frame(
    x1 = rnorm(150, mean = 10, sd = 2),
    x2 = rnorm(150, mean = 20, sd = 3),
    cat1 = factor(rep(c("A", "B"), length.out = 150))
  )

  output <- sg_vpop_est(data_i = test_data, seed = 555,
                        diag_plots = TRUE, seed_umap = 42)
...
test_that("sg_vpop_est respects tg_corrdif in search mode", {
  ...
  output <- sg_vpop_est(data_i = test_data, seed = NA, nds = 2,
                        tg_corrdif = 0.05, diag_plots = FALSE)
...
test_that("sg_vpop_est handles categorical-only data", {
  ...
  test_data <- data.frame(
    ...
    cat3 = factor(sample(c("Low", "Med", "High"), 100, replace = TRUE))
  )

  output <- sg_vpop_est(data_i = test_data, seed = 999, diag_plots = FALSE)
...
test_that("sg_vpop_est noise_level parameter affects duplicate removal", {
  ...
  output1 <- sg_vpop_est(data_i = test_data, seed = 123,
                         remove_duplicates = TRUE, noise_level = 0.05,
                         diag_plots = FALSE)

  output2 <- sg_vpop_est(data_i = test_data, seed = 123,
